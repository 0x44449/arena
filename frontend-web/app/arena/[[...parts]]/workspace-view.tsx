  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      <div className="h-12 border-b border-[#E5E7EB] flex items-center justify-between px-4 bg-white">
        ...
        <button
          onClick={handleTeamSettingClick}
          className="w-6 h-6 flex items-center justify-center rounded hover:bg-[#F3F4F6] text-[#6B7280] hover:text-[#374151] transition-colors duration-150"
        >
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="p-2">
          <div className="space-y-0.5">
            <button
              className={cn(
                "w-full flex items-center space-x-3 px-2 py-1.5 rounded text-sm transition-colors duration-150",
                false
        ...
        {workspaces?.map(workspace => (
          <WorkspaceButton key={workspace.workspaceId} workspace={workspace} />
        ))}
      </div>
      <WorkspaceAddModal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} />
    </div>
  )
}