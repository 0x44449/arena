...
import { IssueLiveStsDto } from "./dtos/issue-live-sts.dto";
import { LiveStsDto } from "./dtos/live-sts.dto";
...
export class AuthController {
  ...
  @Post("live/token")
  @ApiOkResponse({ type: () => withApiResult(LiveStsDto) })
  async issueLiveToken(@Body() body: IssueLiveStsDto, @ReqCredential() credential: ArenaWebCredential): Promise<ApiResultDto<LiveStsDto>> {
    const sts = await this.authService.issueLiveSTS(body.channelId, credential.user!);

    return new ApiResultDto<LiveStsDto>({ data: { sts } });
  }
}