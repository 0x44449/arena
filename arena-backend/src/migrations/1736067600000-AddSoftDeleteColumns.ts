...
export class AddSoftDeleteColumns1736067600000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "channels" ADD "deletedAt" TIMESTAMP WITH TIME ZONE`,
    );

    // messages: deletedAt 추가
    await queryRunner.query(
      `ALTER TABLE "messages" ADD "deletedAt" TIMESTAMP WITH TIME ZONE`,
    );

    // participants: joinedAt → createdAt 이름 변경, updatedAt, deletedAt 추가
    await queryRunner.query(
      `ALTER TABLE "participants" RENAME COLUMN "joinedAt" TO "createdAt"`,
    );
    await queryRunner.query(
      `ALTER TABLE "participants" ADD "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()`,
    );
    await queryRunner.query(
      `ALTER TABLE "participants" ADD "deletedAt" TIMESTAMP WITH TIME ZONE`,
    );

    // contacts: updatedAt, deletedAt 추가
    await queryRunner.query(
      `ALTER TABLE "contacts" ADD "updatedAt" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()`,
    );
    await queryRunner.query(
      `ALTER TABLE "contacts" ADD "deletedAt" TIMESTAMP WITH TIME ZONE`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "participants" DROP COLUMN "deletedAt"`,
    );
    await queryRunner.query(
      `ALTER TABLE "participants" DROP COLUMN "updatedAt"`,
    );
    await queryRunner.query(
      `ALTER TABLE "participants" RENAME COLUMN "createdAt" TO "joinedAt"`,
    );
